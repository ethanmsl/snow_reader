...
{
        println!("--------------------------------");
        let contents = snow_read::snow_example_long::<&str>(None)?;
        let item_long: Unload = serde_xml_rs::from_str(&contents).expect("Failed to parse `&contents` into `Unload`");
        let contents = snow_read::snow_example_search();
        let item_search: Unload = serde_xml_rs::from_str(&contents).expect("Failed to parse `&contents` into `Unload`");
